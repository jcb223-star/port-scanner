...
`.github/workflows/tests.yml` pins `actions/checkout@v4`, `actions/setup-python@v5`, and `codecov/codecov-action@v4`, which as of Aug 2026 target the now-deprecated Node 20 runtime (GitHub currently forces them onto Node 24 instead, with a CI annotation each run) — bump to their next major versions next time this workflow is touched.
